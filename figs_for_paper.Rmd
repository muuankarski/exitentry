...
```{ryouthUnemploymentPlotD, results='hide'}
library(ggplot2)
plot <- ggplot(neetL, aes(x=cntry,y=valueNum,
                          fill=variable, label=valueNum)) +
...
ggsave(plot,file="figure/NEETunEmploymentD.png",
       width=14/2.54, 
       height=12/2.54, 
       dpi=300)
```

![](figure/NEETunEmploymentD.png)

- [For MS Word](figure/NEETunEmploymentD.png)

## As stacked barplot

```{ryouthUnemploymentPlotS, results='hide'}
library(ggplot2)
plot <- ggplot(neetL, aes(x=cntry,y=valueNum,
                          label=valueNum,fill=variable,
                          order=variable)) +
  geom_bar(stat="identity", position="stack") +
  theme_minimal() +
  geom_text(vjust=-.2,
            position=position_stack(width = 1),
            size=2) +
  scale_fill_manual(values = cbbPalette) + 
  labs(x="",y="15-29 year-olds who are neither \n in employment nor in education or training") +
  theme(legend.title=element_blank()) +
...
ggsave(plot,file="figure/NEETunEmploymentS.png",
       width=14/2.54, 
       height=12/2.54, 
       dpi=300)
```

![](figure/NEETunEmploymentS.png)

- [For MS Word](figure/NEETunEmploymentS.png)
...
```{rsubsetentermaterintyhistogramFirstBirth}
rm()
load("data/datEnterBirth.RData")
load("data/cntryDat.RData")

datTm <- datEnterBirth[datEnterBirth$firstBirth == 1, ]
datTm$dup  <- duplicated(datTm$RB030)
datTm <- datTm[datTm$dup == FALSE,]
library(ggplot2)
library(grid)
...
ggsave(plot,file="figure/MotherAgeFB.png",
       width=14/2.54, 
       height=10/2.54, 
       dpi=300)
```
...
![](figure/MotherAgeFB.png)

- [For MS Word](figure/MotherAgeFB.png)
...
```{rsurvivalPostSoc}
df.long5 <- df.long4[df.long4$regime %in% c("Central Europe","Nordic","Southern Europe","Post Soc"),]
...
ggsave(plot,file="figure/KaplanMeierPS.png",
       width=14/2.54, 
       height=10/2.54, 
       dpi=300)

ggsave(plotCI,file="figure/KaplanMeierCIPS.png",
       width=14/2.54, 
       height=10/2.54, 
       dpi=300)
```
...
![](figure/KaplanMeierPS.png)

[Load for Ms Word](figure/KaplanMeierPS.png)

### With CI

![](figure/KaplanMeierCIPS.png)

[Load for Ms Word](figure/KaplanMeierCIPS.png)