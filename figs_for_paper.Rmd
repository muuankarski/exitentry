...
# Old age dependency rations in Europe 2010 - 2060
...
plot <- ggplot(datOe, aes(x=time,y=value,color=regime,group=geo)) +
  ...
  annotate("rect", xmin = 2012, xmax = 2060, 
    ymin = 5, ymax = 65, alpha = 0.1) +
  theme_minimal() +
  scale_color_manual(values = cbbPalette) + 
  annotate("text", x = 2040, y = 15, label = "Projected",size=2) +
  labs(x="year",y="Old age dependency ration") +
  theme(legend.position="top") +
  geom_text(data=merge(datOe, aggregate(time ~ geo, datOe, max),
                            by=c("time","geo")),
                 aes(x=time,y=value,label=cntry),
                 hjust=-.1,size=2) +
  coord_cartesian(xlim=c(1990,2070)) +
