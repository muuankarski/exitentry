...
opts_chunk$set(echo=FALSE,eval=TRUE, cache=TRUE,warning=FALSE,message=FALSE,fig.width=12,fig.height=8)
```
...
--------  -------------------
20131022  created
20131023  Reshaped the data suitable for Cox ph models and survival curves. Some descriptive graphics along the way
...
library(stringr)
df.long3$startDate <- as.Date(12753 + (as.numeric(str_replace(df.long3$var_start, "start", "")))*31)
df.long3$endDate <- as.Date(12753 + (as.numeric(str_replace(df.long3$var_end, "end", "")))*31)
