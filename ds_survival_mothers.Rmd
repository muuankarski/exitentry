...
save(df.long3, file="data/survival.RData")
...
library(simPH)
kmsurvival2 <- survfit(Surv(duration,event) ~ RB020, 
                       data = df.long3)
ggfitStrata(kmsurvival2, byStrata = TRUE, 
            title="Stratified survival curves from Cox Proportional Hazards model")
```
...
#kmsurvival <- survfit(Surv(df.long3$duration,df.long3$event) ~ 1)
df.long3$cntry <- NULL
df.long3 <- merge(df.long3,cntryDat,by.x="RB020",
                   by.y="iso2c")
```
...
```{rsurvTablesRegime}
reg <- survfit(Surv(duration,event) ~ regime, 
                       data = df.long3)
summary(reg)
```
...
```{rsurvTablesCntry}
cnt <- survfit(Surv(duration,event) ~ cntry, 
                       data = df.long3)
summary(cnt)
```
