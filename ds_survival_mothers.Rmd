...
save(df.long3, file="data/df.long3.RData")
...
df.long3 <- merge(df.long3,cntryDat,by.x="RB020",
                        by.y="iso2c")
tbl <- data.frame(table(df.long3$regime,df.long3$event))
...
## Regime level (still useless..)

```{rRegimeTesti}
source("survival/ggkm.R")
reg <- survfit(Surv(duration,event) ~ regime, 
                       data = df.long3)
ggkm(reg, timeby=500)
