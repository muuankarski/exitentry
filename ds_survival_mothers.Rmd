...
library(stringr)
library(zoo)
df.long3$startDate <- as.Date(12753 + 
                                (as.numeric(str_replace(df.long3$var_start, 
                                                        "start","")))*31)
df.long3$endDate <- as.Date(12753 + 
                              (as.numeric(str_replace(df.long3$var_end,
                                                      "end", "")))*31)
df.long3$duration <- as.numeric(df.long3$endDate - 
                                  df.long3$startDate)/31
#df.long3 <- df.long3[df.long3$duration > 0,]

df.long3$duration[df.long3$duration <= 0] <- NA

df.long3$event[df.long3$value_end == 1] <- TRUE
df.long3$event[is.na(df.long3$value_end)] <- FALSE
