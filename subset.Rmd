...
opts_chunk$set(echo=FALSE,eval=TRUE, cache=TRUE,warning=FALSE)
```
...
datEntW <- merge(df.long[,c("RB020","RB030","year","schoolExit")],dat.per, 
                  by=c("RB020","RB030"), all.x=TRUE)

# yritäppä saa joku roti siihen, missä on vika kun vuosiin tulee NA arvo ja muutenkin on väjhän sotkua..
...
datTm4 <- datEntW[datEntW$year==(datEntW$RB010+4),]
datTm5 <- datEntW[datEntW$year==(datEntW$RB010+5),]
# after
...
datT <- datT[, c("PER_ID","PY010G","RB020","HX100","year")]
names(datT) <- c("PER_ID","incomeT","RB020","HX100","year")
...
datTm4 <- datTm4[, c("PER_ID","PY010G","RB020")]
names(datTm4) <- c("PER_ID","incomeTmin4","RB020")
datTm5 <- datTm5[, c("PER_ID","PY010G","RB020")]
names(datTm5) <- c("PER_ID","incomeTmin5","RB020")
...
dat.plot <- merge(dat.plot,datTm4,by=c("PER_ID","RB020"), all.x=TRUE)
dat.plot <- merge(dat.plot,datTm5,by=c("PER_ID","RB020"), all.x=TRUE)
...
- dotted vertcal red line stands for **first year as a "worker"**!


```{rsubsetentryworkplotAbso, fig.height=30, fig.width=12, warning=FALSE}
library(reshape2)
dat.plot.l <- melt(dat.plot.abso, id.vars = c("PER_ID","RB020","HX100"))
dat.plot.l$variable <- factor(dat.plot.l$variable, 
                              levels=c("incomeTmin5",
                                       "incomeTmin4","incomeTmin3",
                                       "incomeTmin2","incomeTmin1",
                                       "incomeT",
                                       "incomeTplus1","incomeTplus2",
                                       "incomeTplus3"))
summary(factor(dat.plot.l$variable))
library(ggplot2)
ggplot(dat.plot.l, aes(x=variable,y=value,group=PER_ID)) +
  ...
  geom_vline(xintercept = 4, color="red", type="dashed")
...
facet_wrap(~RB020, ncol=3) +
  labs(title="Change in relative Employee cash or near cash income(gross) when leaving school for work") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5)) +
  annotate("text", y=1500, x="incomeT", label="First year \n at work") +
