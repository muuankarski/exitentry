...
df.wide <- dcast(dat.per, PER_ID + RB020 + 
                   RB030 + HX100 + HX090 + PY010G ~ RB010, 
                 value.var = "econStatus")

names(df.wide) <- c("PER_ID","RB020","RB030","HX100","HX090","PY010G",
                    "x2005","x2006","x2007",
                    "x2008","x2009","x2010")

df.wide$school06[df.wide$x2005 == 6 & df.wide$x2006 %in% 1:4] <- 1
df.wide$school06[is.na(df.wide$school06)] <- 0
summary(factor(df.wide$school06))
summary(df.wide[df.wide$school06 == 1,])

ddply(df.wide, "school06", summarise,
               N    = length(PY010G),
               mean = mean(PY010G, na.rm=TRUE),
               median   = sd(PY010G, na.rm=TRUE))

df.wide$school07[df.wide$x2006 == 6 & df.wide$x2007 %in% 1:4] <- 1
df.wide$school07[is.na(df.wide$school07)] <- 0
summary(factor(df.wide$school07))
summary(df.wide[df.wide$school07 == 1,])

ddply(df.wide, "school07", summarise,
               N    = length(PY010G),
               mean = mean(PY010G, na.rm=TRUE),
               median   = sd(PY010G, na.rm=TRUE))
...
summary(factor(df.wide$school08))
summary(df.wide[df.wide$school08 == 1,])

ddply(df.wide, "school08", summarise,
               N    = length(PY010G),
               mean = mean(PY010G, na.rm=TRUE),
               median   = sd(PY010G, na.rm=TRUE))
...
summary(factor(df.wide$school09))
summary(df.wide[df.wide$school09 == 1,])

ddply(df.wide, "school09", summarise,
               N    = length(PY010G),
               mean = mean(PY010G, na.rm=TRUE),
               median   = sd(PY010G, na.rm=TRUE))
...
summary(factor(df.wide$school10))
summary(df.wide[df.wide$school10 == 1,])

ddply(df.wide, "school10", summarise,
               N    = length(PY010G),
               mean = mean(PY010G, na.rm=TRUE),
               median   = sd(PY010G, na.rm=TRUE))
...
df.long <- melt(df.wide, id.vars =c("PER_ID",
                                    "RB020","RB030","HX100","HX090","PY010G"), 
...
summary(factor(df.long$schoolExit))

library(plyr)
ddply(df.long, "schoolExit", summarise,
               N    = length(PY010G),
               mean = mean(PY010G, na.rm=TRUE),
               median   = sd(PY010G, na.rm=TRUE))

df.longx <- df.long[df.long$schoolExit == 1, ] # all we need are the (ex)-students

datExitStudies <- merge(df.longx[,c("RB020","RB030","year","schoolExit")],dat.per, 
...
```{rrsubsetentryworkplotAbsobox, fig.height=6, fig.width=12, warning=FALSE, eval=TRUE}
ggplot(data=datExitStudies, 
       aes(x=factor(time),y=PY010G)) + 
  geom_boxplot() +
  labs(title="Employee cash or near cash income(gross) years before and after exiting to work") +
```
...
```{rsubsetentryworkplotRela, fig.height=30, fig.width=12, warning=FALSE, eval=FALSE}
library(reshape2)
datExitStudies$time <- factor(datExitStudies$time)
datExitStudies$PY010G[is.na(datExitStudies$PY010G)] <- 1
...
```{rsubsetfiglabour1, fig.height=32, fig.width=12, eval=TRUE}
library(ggplot2)
ggplot(data=datExitStudies, 
