...
```{ropts}
opts_chunk$set(echo=FALSE,eval=FALSE, cache=TRUE)
```
...
In this file the exit/entry events are coded. Main source of information on the events are variables [PL030](http://www.gesis.org/?id=8063#PL030) (*Self-defined current economic status*) and [RB110](http://www.gesis.org/?id=8062#RB110) (*Membership status*).

Table: values and labels in variable [PL030](http://www.gesis.org/?id=8063#PL030) (actually PL031 !!)
...
Table: values and labels in variable [RB110](http://www.gesis.org/?id=8062#RB110) (*Membership status*)
...
```{rsubsetloadmergedhouseholdfiles,eval=FALSE}
...
#save(dat.hh, file="data/dat.hh.RData")
head(dat.hh)
```
...
Based on the analysis above, I have create a variable called `educ`with value `1` for education and `0` for non-education. It is calculated from variable [`PY140G/PY140N`](http://www.gesis.org/?id=8063#PY140G) (Education-related allowances) in a following manner

- if value is 0 or less or NA -> `0`
- other values -> `1`

```{rsubsetdefinestudent}
#######
load("data/dat.per.RData")
####### 
dat.per$educ[dat.per$PY140G <= 0] <- 0
dat.per$educ[is.na(dat.per$PY140G)] <- 0
dat.per$educ[dat.per$PY140G > 0] <- 1
# what we got
summary(factor(dat.per$educ))
```

So, we have the variable now, then we need to find out **whose status has changed from 1 to 0 at AND who is full- or part-time employed at that time**?

Let's use the `educ`-variable and [RB210](http://www.gesis.org/?id=8062#RB210) variable that get's value 1 if person is at work.
...
So we reason that if person is not reveiving student benefits and is at work, then his/her main status is `at work = 1`. If  


```{rsubsetrecodeschool}
#dat.per$status <- NULL
dat.per$status[dat.per$educ == 1 & dat.per$RB210 == 4] <- 5
dat.per$status[dat.per$educ == 0 & dat.per$RB210 == 1] <- 1
dat.per$status[dat.per$RB210 == 2] <- 2
dat.per$status[dat.per$RB210 == 3] <- 3
dat.per$status[dat.per$educ == 0 & dat.per$RB210 == 4] <- 4
...
names(df.wide) <- c("PER_ID","RB020","RB030",
                    "x2005","x2006","x2007",
                    "x2008","x2009","x2010")
df.wide$school06[df.wide$x2005 == 5 & df.wide$x2006 == 1] <- 1
df.wide$school07[df.wide$x2006 == 5 & df.wide$x2007 == 1] <- 1
df.wide$school08[df.wide$x2007 == 5 & df.wide$x2008 == 1] <- 1
df.wide$school09[df.wide$x2008 == 5 & df.wide$x2009 == 1] <- 1
df.wide$school10[df.wide$x2009 == 5 & df.wide$x2010 == 1] <- 1
...
datT <- datT[, c("PER_ID","PY010N","RB020")]
names(datT) <- c("PER_ID","incomeT","RB020")
datTm1 <- datTm1[, c("PER_ID","PY010N","RB020")]
names(datTm1) <- c("PER_ID","incomeTmin1","RB020")
datTm2 <- datTm2[, c("PER_ID","PY010N","RB020")]
names(datTm2) <- c("PER_ID","incomeTmin2","RB020")
datTm3 <- datTm3[, c("PER_ID","PY010N","RB020")]
names(datTm3) <- c("PER_ID","incomeTmin3","RB020")
datTp1 <- datTp1[, c("PER_ID","PY010N","RB020")]
names(datTp1) <- c("PER_ID","incomeTplus1","RB020")
datTp2 <- datTp2[, c("PER_ID","PY010N","RB020")]
names(datTp2) <- c("PER_ID","incomeTplus2","RB020")
datTp3 <- datTp3[, c("PER_ID","PY010N","RB020")]
names(datTp3) <- c("PER_ID","incomeTplus3","RB020")

dat.plot <- merge(datT,datTm1,by=c(c("PER_ID","RB020"),"RB020"), all.x=TRUE)
...
### Plotting the change in employee cash income due to entering work

In the figure below `IncomeT` stands for time point when *event* took place. `IncomeTmin1`is then one year before and `IncomeTplus1` is one year after and so forth.

All very tentative..

```{rsubsetentryworkplot, fig.height=14, fig.width=14, warning=FALSE}
ggplot(dat.plot.l, aes(x=variable,y=value,group=PER_ID)) +
geom_point() + geom_line() +
coord_cartesian(ylim=c(0,2000)) +
facet_wrap(~RB020) +
  labs(title="Change in Employee cash or near cash income(net) when leaving school for work") +
...
```{rsubsetexitbirthrecode}
dfz <- dat.per.birth[!is.na(dat.per.birth$birthGiven), ]
head(df)
df <- dfz[,c("RB020","PER_ID","RB010","year","PY010N","birthGiven")]
...
```{rsubsetexitbirthplot, fig.height=14, fig.width=14, warning=FALSE}
ggplot(dat.plot.l, aes(x=variable,y=value,group=PER_ID)) +
geom_point(alpha=.2) + geom_line(alpha=.2) +
coord_cartesian(ylim=c(0,2000)) +
facet_wrap(~RB020) +
  labs(title="Change in Employee cash or near cash income(net) when giving birth") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5))
...
names(df.wide) <- c("PER_ID","RB020","RB030",
                    "x2005","x2006","x2007",
                    "x2008","x2009","x2010")
df.wide$school06[df.wide$x2005 != 3 & df.wide$x2006 == 3] <- 1
df.wide$school07[df.wide$x2006 != 3 & df.wide$x2007 == 3] <- 1
df.wide$school08[df.wide$x2007 != 3 & df.wide$x2008 == 3] <- 1
df.wide$school09[df.wide$x2008 != 3 & df.wide$x2009 == 3] <- 1
df.wide$school10[df.wide$x2009 != 3 & df.wide$x2010 == 3] <- 1
...
```{rsubsetexitpensionplot, fig.height=14, fig.width=14, warning=FALSE}
ggplot(dat.plot.l, aes(x=variable,y=value,group=PER_ID)) +
geom_point() + geom_line() +
coord_cartesian(ylim=c(0,2000)) +
facet_wrap(~RB020) +
  labs(title="Change in Employee cash or near cash income(net) when exiting for pension") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5))
...
```{rsubsetpensionplot, fig.height=14, fig.width=14, warning=FALSE}
ggplot(data=datBabyPlot, 
       aes(x=RB020,y=RX010)) + 
  geom_boxplot() +
