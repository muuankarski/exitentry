...
1. merging the original register and data files both at the household and personal level from year 2008, 2009, 2010 and 2011 (each containing data from that year and three years before)
...
df.wide$school11[df.wide$x2010 == 6 & df.wide$x2011 %in% 1:4] <- 1
df.wide$school11[is.na(df.wide$school11)] <- 0
...
df.long$year[df.long$year == "school11"] <- "2011"
...
save(datExitStudies, file="data/datExitStudies.RData")
```
...
datTe <- merge(datTe,regime,by.x="RB020",by.y="NUTS_ID")
library(ggplot2)
ggplot(data=datTe, 
...
datEnterBirth <- merge(dat.per,newborn[,c("year","RB020",
                                          "RB230","birthGiven")],
                       by.x=c("RB020","RB030"),
                       by.y=c("RB020","RB230"))
# wave relative to birth of baby
...
## First birth
datEnterBirth$childNr[datEnterBirth$HX050 == 1] <- 0
datEnterBirth$childNr[datEnterBirth$HX050 == 1.5] <- 0
datEnterBirth$childNr[datEnterBirth$HX050 == 1.3] <- 1
datEnterBirth$childNr[datEnterBirth$HX050 == 1.3] <- 1
datEnterBirth$childNr[datEnterBirth$HX050 == 1.8] <- 1
datEnterBirth$childNr[datEnterBirth$HX050 == 2.3] <- 1
datEnterBirth$childNr[datEnterBirth$HX050 == 2.8] <- 1
datEnterBirth$childNr[datEnterBirth$HX050 == 3.3] <- 1
datEnterBirth$childNr[datEnterBirth$HX050 == 3.8] <- 1
datEnterBirth$childNr[datEnterBirth$HX050 == 4.3] <- 1
datEnterBirth$childNr[datEnterBirth$HX050 == 1.6] <- 2
datEnterBirth$childNr[datEnterBirth$HX050 == 2.1] <- 2
datEnterBirth$childNr[!(datEnterBirth$childNr %in% 0:2)] <- 3
                    
datEnterBirth$firstBirth <- ifelse(datEnterBirth$childNr == 0 & datEnterBirth$time == -1, 1, 0)
#
library(reshape2)
datEnterBirth.l <- dcast(datEnterBirth, RB020 + PER_ID + firstBirth ~ time, value.var="RX010")
names(datEnterBirth.l) <- c("RB020","PER_ID",
                            "firstBirth","xm5",
                            "xm4","xm3","xm2",
                            "xm1","x0","xp1",
                            "xp2","xp3",
...
cond <- cond[,1:3]
datEnterBirth2 <- merge(cond,datEnterBirth,by=c("RB020","PER_ID","firstBirth"))
#
save(datEnterBirth, file="data/datEnterBirth.RData")
save(datEnterBirth2, file="data/datEnterBirth2.RData")
# for entry exit analysis we want only the from the year baby was born
datEnterBirth3 <- datEnterBirth2[datEnterBirth2$time >= 0,]
save(datEnterBirth3, file="data/datEnterBirth3.RData")
# data for sequence analysis
datEnterBirth4 <- datEnterBirth2[datEnterBirth2$time >= -1,]
save(datEnterBirth4, file="data/datEnterBirth4.RData")
```
...
library(ggplot2)
datTm <- merge(datTm,regime,by.x="RB020",by.y="NUTS_ID")
ggplot(data=datTm, 
       aes(x=factor(reorder(RB020, RX010, median, na.rm=TRUE)),
           y=RX010,
           fill=regime_en)) + 
...
df.wide$pension11[df.wide$x2010 != 7 & df.wide$x2011 == 7] <- 1
df.wide$pension11[is.na(df.wide$pension11)] <- 0
...
df.long$year[df.long$year == "pension11"] <- "2011"
