...
```{ropts}
opts_chunk$set(echo=FALSE,eval=TRUE, cache=TRUE)
```
...
names(df.wide) <- c("PER_ID","RB020","RB030","RB040",
                    "x2005","x2006","x2007",
                    "x2008","x2009","x2010")

df.wide$school06[df.wide$x2005 == 6 & df.wide$x2006 == 1:4] <- 1
df.wide$school07[df.wide$x2006 == 6 & df.wide$x2007 == 1:4] <- 1
df.wide$school08[df.wide$x2007 == 6 & df.wide$x2008 == 1:4] <- 1
df.wide$school09[df.wide$x2008 == 6 & df.wide$x2009 == 1:4] <- 1
df.wide$school10[df.wide$x2009 == 6 & df.wide$x2010 == 1:4] <- 1
...
df.long <- melt(df.wide, id.vars =c("PER_ID",
                ...
                measure.vars=c("school06","school07",
                               "school08","school09",
                               "school10"))
names(df.long) <- c("PER_ID","RB020","RB030","RB040",
                    "year","workEntry")
...
datT <- datT[, c("PER_ID","PY010N","RB020","RB040","year")]
names(datT) <- c("PER_ID","incomeT","RB020","RB040","year")
...
dat.plot <- dat.plot[,c(1,2,3,6,7,8,9,10,11,4,5)]
...
**Missing countries are missing because of missing data in income variable!!**

**Change in absolute income**

```{rsubsetentryworkplotAbso, fig.height=30, fig.width=12, warning=FALSE}
library(reshape2)
...
facet_wrap(~RB020, ncol=3) +
  labs(title="Change in absolute Employee cash or near cash income(net) when leaving school for work") +
...
```{rsubsetentryworkplotRela, fig.height=30, fig.width=12, warning=FALSE}
library(reshape2)
...
library(ggplot2)
dat.plot <- datEntW[datEntW$year==datEntW$RB010,]

ggplot(data=dat.plot, 
       aes(x=factor(RB010),y=RX010)) + 
  geom_boxplot() +
  labs(title="Age of individual entering labour market") +
  facet_wrap(~RB020, ncol=3)
...
```{rsubsetfiglabour2, fig.height=15, fig.width=15}
library(ggplot2)
ggplot(data=dat.plot, 
...
dat.per <- dat.per[dat.per$RB090 == 2,]
...
head(df)
datEntW <- dfz[,c("RB020","PER_ID","RB010","year","PY010N","birthGiven")]
datT <- datEntW[datEntW$year==datEntW$RB010,]
# before
datTm1 <- datEntW[datEntW$year==(datEntW$RB010+1),]
datTm2 <- datEntW[datEntW$year==(datEntW$RB010+2),]
...
coord_cartesian(ylim=c(0,50000)) +
facet_wrap(~RB020, ncol=3) +
  labs(title="Change in absolute Employee cash or near cash income(net) when giving birth") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5))
...
coord_cartesian(ylim=c(0,2000)) +
facet_wrap(~RB020, ncol=3) +
  labs(title="Change in relative Employee cash or near cash income(net) when giving birth") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5))
...
coord_cartesian(ylim=c(0,50000)) +
facet_wrap(~RB020, ncol=3) +
  labs(title="Change in absolute Employee cash or near cash income(net) when exiting for pension") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5))
...
coord_cartesian(ylim=c(0,2000)) +
facet_wrap(~RB020, ncol=3) +
  labs(title="Change in relative Employee cash or near cash income(net) when exiting for pension") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5))
